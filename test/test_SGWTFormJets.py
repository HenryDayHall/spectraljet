...
def test_SGWT_run():
    ...
    pys = np.array([])
    pzs = np.array([])
    algo = SGWTFormJets.SGWT.from_kinematics(energies, pxs, pys, pzs,
...
def test_SGWT_complex(n_particles=10):
    # this test is expected to break if the algorithm is changed
    np.random.seed(42)
    energies = np.random.rand(n_particles)*10. + 2.
    pxs = np.random.rand(n_particles)*2. - 1.
    pys = np.random.rand(n_particles)*2. - 1.
    pzs = np.random.rand(n_particles)*2. - 1.
    algo = SGWTFormJets.SGWT.from_kinematics(energies, pxs, pys, pzs,
                                             dict_jet_params = dict(NRounds=15, Sigma=0.1, Cutoff=0.))
    algo.run()
